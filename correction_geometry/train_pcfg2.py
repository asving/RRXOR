"""PCFG-2 (RHM) training: dense vs hard-only (class b >= 2)."""
import argparse, json, os
import torch
import torch.nn.functional as F
from train_multi import GPTV
from pcfg2 import gen_batch_pcfg2, CLS, NLEAF, NTREE, V
...
def main():
    ...
    name = f"pcfg2_{args.mask}_seed{args.seed}"
    ...
    eseq = gen_batch_pcfg2(4096, g_ev)
    ...
    json.dump({"mask": args.mask, "seed": args.seed, "L": L, "nl": nl, "vocab": V,
               "steps": args.steps, "batch": Bt, "lr": 3e-4, "gseed": 7},
    ...
    for step in range(args.steps + 1):
        seq = gen_batch_pcfg2(Bt, g).to(dev)
        lg = model(seq[:, :-1])
