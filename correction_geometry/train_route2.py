...
def gen(B, m1, m2, arm, g, conflict=False, mask1=False):
...
def main():
    ...
    ap.add_argument("--arm", choices=["ll1", "lx", "xl"], required=True)
    ...
    dev = "cuda"
    name = f"r2_{args.arm}_m{args.m1}x{args.m2}_s{args.seed}" + (f"_x{args.cross}" if args.cross else "")
    run = os.path.join(os.path.dirname(os.path.abspath(__file__)), "runs", name)
    os.makedirs(run, exist_ok=True)
    g_ev = torch.Generator().manual_seed(4242)
    ev, evm = gen(512, args.m1, args.m2, args.arm, g_ev)
    ...
    for step in range(args.steps + 1):
        ph1 = step < args.cross
        seq, _ = gen(128, 0 if ph1 else args.m1, args.m2, args.arm, g, mask1=ph1)
        seq = seq.to(dev)
        lg = model(seq[:, :-1])
