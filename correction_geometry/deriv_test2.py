"""Control: same derivative test on PCFG-2 (deterministic RHM, leap-hard)."""
import glob, json
import torch
from train_multi import GPTV
from pcfg2 import V, NTREE
from pcfg3 import soft_tables
from pcfg4 import bp4

P = soft_tables(1e-9)
dev = "cuda"
ev = torch.load("runs/pcfg2_dense_seed0/evalset.pt")
...
for lev in range(1, 4):
    ...
    pb = pi * lam[lev][:, child]
    TARG[4 - lev] = pb / pb.sum(-1, keepdim=True).clamp_min(1e-30)

model = GPTV(L=96, nl=6, vocab=V).to(dev)
cks = sorted(glob.glob("runs/pcfg2_dense_seed0/ckpts/*.pt"))
...
print("pcfg2 probe R2 final (order 1..4):", " ".join(f"{final[r]:.3f}" for r in (1, 2, 3, 4)))
...
def deriv_reg(c_r, c_prev, ts, tag):
    ...
    print(f"  {tag}: n={n}  slope b={b:+.2e}  corr={r:+.2f}  "
          f"{'MULTIPATH (b>0)' if b > 0 and abs(r) > 0.5 else 'parallel-consistent'}")
print("pcfg2 derivative test:")
for r in (2, 3, 4):
